<script lang="ts">
  ...
  let exporting = false;
  let search = '';
  let filter: 'all' | 'annotated' | 'unannotated' | 'empty' = 'all';
  ...
  $: filteredImages = (project?.images ?? []).filter((image) => {
    ...
    const matchesFilter = filter === 'all' || (filter === 'annotated' && image.annotated && !image.empty) || (filter === 'unannotated' && !image.annotated) || (filter === 'empty' && image.empty);
  ...
  async function loadRuntimeProject(query = '') {
    ...
    try {
      ...
      const response = await fetch(`/__boxscribe/project?limit=200&query=${encodeURIComponent(query)}${classQuery}`);
  ...
  async function preloadNeighbor(current: typeof project.images[number], direction: number) {
    if (!runtimeDataset) return;
    let image: typeof project.images[number] | undefined;
    if (classFilters.length || search) {
      const classQuery = classFilters.length ? `&classIds=${classFilters.join(',')}` : '';
      const response = await fetch(`/__boxscribe/neighbor?id=${current.index}&direction=${direction}&query=${encodeURIComponent(search)}${classQuery}`);
      if (!response.ok) return; image = await response.json();
    } else {
      const index = (current.index ?? 0) + direction;
      if (index < 0 || index >= totalImages) return;
      image = project.images.find((item) => item.index === index);
      if (!image) { const response = await fetch(`/__boxscribe/item?index=${index}`); if (!response.ok) return; image = await response.json(); }
    }
  ...
  async function save() {
    ...
    try {
      ...
      if (item) { item.annotated = true; item.empty = boxes.length === 0; item.boxCount = boxes.length; project = { ...project, annotatedCount: wasAnnotated ? project.annotatedCount : (project.annotatedCount ?? annotatedCount) + 1, images: [...project.images] }; }
  ...
  async function navigate(direction: number) {
    ...
    if (runtimeDataset && current && (classFilters.length || search)) {
      const classQuery = classFilters.length ? `&classIds=${classFilters.join(',')}` : '';
      const response = await fetch(`/__boxscribe/neighbor?id=${current.index}&direction=${direction}&query=${encodeURIComponent(search)}${classQuery}`);
  ...
  async function nextUnannotated() {
    ...
    const next = [...project.images.slice(start + 1), ...project.images.slice(0, start)].find((image) => !image.annotated);
    if (next) await openImage(next.id);
  }
  ...
  function keyHandler(event: KeyboardEvent) {
    ...
    else if (shortcuts.focus.includes(key as never) && selectedId) canvas?.focusSelected();
...
<main class="app-shell">
    <header class="topbar">
      <div class:complete={progress === 100} class="top-progress">
        <div class="progress-head"><span>Размечено</span><strong title={`${formatCount(annotatedCount)} из ${formatCount(totalImages)}`}>{formatCount(annotatedCount)}</strong><b>{progress}%</b></div>
    ...
    <aside class="sidebar">
      ...
      {:else}
        <div class="status-filter"><span>Статус</span><div class="filters"><button class:active={filter === 'all'} on:click={() => filter = 'all'}>Все</button><button class:active={filter === 'unannotated'} on:click={() => filter = 'unannotated'}>Новые</button><button class:active={filter === 'annotated'} on:click={() => filter = 'annotated'}>Готово</button><button class:active={filter === 'empty'} on:click={() => filter = 'empty'}>Пустые</button></div></div>
      {/if}
      <div class="image-list" bind:this={imageList}>
        {#each filteredImages as image, index}
          <button data-image-id={image.id} class:active={image.id === currentId} on:click={() => openImage(image.id)}>
            <span class:done={image.boxCount > 0} class:empty={image.boxCount === 0} class:unknown={image.boxCount < 0} class="status">{image.boxCount < 0 ? '?' : image.boxCount}</span>
            <span class="file"><b>{image.name}</b><small>{image.width ? `${image.width}×${image.height}` : 'Данные при открытии'}{image.boxCount ? ` · ${image.boxCount} bbox` : ''}</small></span><span class="number">{String((image.index ?? project!.images.indexOf(image)) + 1).padStart(3, '0')}</span>
          </button>
        {/each}
...
<style>
  ...
  .selection-actions{display:flex;align-items:center;gap:4px}.selection-actions button{width:25px;height:25px;display:grid;place-items:center;border-radius:5px}.selection-actions .focus-selected:hover{color:#e9ff70;background:#e9ff7010}.selection-actions button:last-child:hover{background:#ff8d7210}
